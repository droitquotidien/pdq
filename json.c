...
ssize_t write_json_stripped_chars(int fildes,
				  ...
				  struct write_buffer *wbuf)
{
	...
	for (i = 0; i < len; i++, str++) {
		if (*str == 0) {
			...
			if (bufferlen > 0) {
				CHECK_WRITE(fildes, chunk_start, bufferlen,
					    wbuf)
			}
			   ...
			   struct write_buffer *wbuf, char standalone)
{
	...
	if (contenu->bloc_textuel.size > 0) {
		CONTENT_WRITE(fildes, bloc_textuel, 12,
			      contenu->bloc_textuel.text, wbuf);
	}
	if (contenu->signataires.size > 0) {
		CONTENT_WRITE(fildes, signataires, 11,
			      contenu->signataires.text, wbuf);
	}
...
ssize_t write_tocitem_json(int fildes, struct tocitem *tocitem,
			    ...
			    struct write_buffer *wbuf, char standalone)
{
	...
	for (i = 0; i < versions->nb_versions; i++) {
		...
		r = write_version_json(fildes, version, wbuf, last);
		if (r < 0) return -1;
	}
	if (standalone) {
		CHECK_WRITE(fildes, "]\n", 2, wbuf)
			...
			struct write_buffer *wbuf, char last)
{
	...
	JATTR(fildes, date_signature_texte, 20, lien->date_signature_texte,
	      wbuf);
			 ...
			 struct write_buffer *wbuf, char standalone)
{
	...
	for (i = 0; i < liens->nb_liens; i++) {
		...
		r = write_lien_json(fildes, lien, wbuf, last);
		if (r < 0) return -1;
	}
	if (standalone) {
		CHECK_WRITE(fildes, "]\n", 2, wbuf)
...
ssize_t write_json(struct parsed_data *pdata, int fildes,
		   struct write_buffer *wbuf)
{
	ssize_t r;
	ssize_t rt = 0;
	...
	char *cid =
	    (*mdata->contexte.cid ==
	     0 ? (*mdata->cid == 0 ? NULL : mdata->cid) : mdata->contexte.cid);
	...
	case JORFTEXT_DOCTYPE:
		...
		JATTR(fildes, derniere_modification, 21,
		      mdata->derniere_modification, wbuf);
		...
		write_toc_json(fildes, toc, wbuf, 0);
		write_versions_json(fildes, versions, wbuf, 0);
		break;
	case JORFVERS_DOCTYPE:
		...
		write_mcs_json(fildes, mcs, wbuf, 0);
		write_liens_json(fildes, liens, wbuf, 0);
		write_contexte_json(fildes, &mdata->contexte, wbuf, 0);
		write_contenu_json(fildes, contenu, wbuf, 0);
		// FIXME: Entreprise ?
		break;
	...
	case JORFSCTA_DOCTYPE:
		JATTR(fildes, rid, 3, mdata->rid, wbuf);
		CONTENT_WRITE(fildes, titrefull, 9, mdata->titrefull, wbuf);
		CONTENT_WRITE(fildes, commentaire, 11, mdata->commentaire,
			      wbuf);
		write_contexte_json(fildes, &mdata->contexte, wbuf, 0);
		write_toc_json(fildes, toc, wbuf, 0);
		break;
